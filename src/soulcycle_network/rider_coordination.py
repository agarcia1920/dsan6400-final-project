...
def plan_coordination(riders: dict[str, Rider], network_state: NetworkState, weekly_counts: dict[str, int], rng: np.random.Generator) -> list[CoordinationPair]: #function to plan the coordination
    ...
    for (a, b), _ in ordered: #iterate over the ordered pair scores
        if used_partners.get(a, 0) >= MAX_COORDINATION_PARTNERS_PER_WEEK:
            continue #skip if the partner has already been used too many times
        if used_partners.get(b, 0) >= MAX_COORDINATION_PARTNERS_PER_WEEK:
            continue #skip if the partner has already been used too many times
        ...
        if len(planned) >= MAX_COORDINATED_CLASSES_PER_WEEK * len(active):
